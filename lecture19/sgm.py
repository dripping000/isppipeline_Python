...
def get_path_cost(slice, offset, parameters):
    #见PPT上计算公式,越近值越低,相邻的像素要乘以P1 更远的乘以P2
    """
...
def compute_costs(left, right, parameters, save_images):
    ...
    #统计是比谁和周围像素大小的分布类似,然后将分步通过二进制编码
    sys.stdout.flush()
    dawn = t.time()
    ...
    dusk = t.time()
    print('\t(done in {:.2f}s)'.format(dusk - dawn))
    color.grey_show(left_img_census / np.max(left_img_census))
    color.grey_show(right_img_census / np.max(right_img_census))
    if save_images:
...
def sgm():
    ...
    color.grey_show(left/255)
    color.grey_show(right/255)
    #通过图像匹配运算法计算cost
    ...
    color.grey_show(left_disparity_map / 255)
    color.grey_show(right_disparity_map / 255)
    if save_images:
